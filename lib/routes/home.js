    ...
    options: {
        handler: async (request, h) => {
            ...
            const launchData = {
                ...
                oauth_timestamp: (Math.round((new Date()).getTime() / 1000)).toString(), // we need the unix timestamp in seconds, not milliseconds
            ...
            // catch any weird URI characters that were missed before final encoding
            const escapedLaunchParams = launchParams.join('&').replace(/\(/g, '%28').replace(/\)/g, '%29').replace(/!/g, '%21');
            const baseString = `POST&${encodeURIComponent(launchURL)}&${encodeURIComponent(escapedLaunchParams)}`;

            const secret = `${encodeURIComponent('secret')}&`;
            const hmac = crypto.createHmac('sha1', secret).update(baseString).digest();
            const signature = Buffer.from(hmac).toString('base64'); // OAuth signature is all our params hashed, then converted to base64
